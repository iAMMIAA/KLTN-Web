...
function WriteInfoDrug() {
  ...
  const handleSubmit = (event) => {
    ...
    setError('');
    axios.post('http://localhost:3001/write-info-drug', formPaper) 
  ...
  return (
      <div className={`Write-Info-Drug`}>
        ...
        <form className={`form-info-drug`} onSubmit={handleSubmit}>
          <input className='id-drug' type='text' name='id_drug' value={formPaper.id_drug} placeholder='Mã thuốc' onChange={handlePaperChange}></input>
          <input className='name-drug' type='text' name='name' value={formPaper.name} placeholder='Tên thuốc' onChange={handlePaperChange}></input>
          
          <label>Nhập thành phần thuốc:</label>
          <ReactQuill
              className={`ReactQuill-drug`}
              value={formPaper.excipients}
              onChange={(value) => handleContentChange("excipients", value)}
              modules={modules}
              formats={formats}
          />
          ...
          <ReactQuill
              ...
              onChange={(value) => handleContentChange("side_effects", value)}
              modules={modules}
              formats={formats}
          />
